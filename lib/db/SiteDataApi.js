...
export async function fetchGlobalAllData({
  ...
  try {
    // 检查是否有匹配当前locale的专属数据库，有则直接用它，跳过默认库
    const matchedSiteId = locale
      ? siteIds.find((sid, idx) => idx > 0 && extractLangPrefix(sid) === locale)
      : null

    if (matchedSiteId) {
      // 直接加载对应语言的数据库，无需先加载默认库
      data = await getSiteDataByPageId({
        pageId: extractLangId(matchedSiteId),
        from
      })
    } else {
      // 默认语言，加载第一个数据库
      const defaultId = extractLangId(siteIds[0])
      data = await getSiteDataByPageId({
        pageId: defaultId,
        from
      })
...
function getCustomMenu({ collectionData, NOTION_CONFIG }) {
  ...
  // 使用 MENU_SORT_ORDER 配置来排序菜单项（slug 列表，SubMenu 紧跟其父 Menu）
  const menuSortOrder = BLOG.MENU_SORT_ORDER || []
  if (menuSortOrder.length > 0) {
    menuPages.sort((a, b) => {
      const ai = menuSortOrder.indexOf(a.slug)
      const bi = menuSortOrder.indexOf(b.slug)
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  ...
  const defaultDescription =
    NOTION_CONFIG?.DESCRIPTION || BLOG.DESCRIPTION
